...
function Get-ServiceAccounts {
    ...
    # NOTE : le filtre est construit en CHAINE (et non en script block { }).
    # Le script block { Name -like $variable } peut ne pas résoudre correctement
    # la variable selon le contexte d'appel (fonction + splatting), ce qui renvoie
    # silencieusement 0 résultat. La chaîne est fiable et reste sûre ici car
    # Get-SanitizedInput interdit déjà les guillemets simples (anti-injection).
    $filterString = "Name -like '$cleanPattern'"
    ...
    try {
        Write-Log -Level INFO -Message "Recherche des comptes de service (filtre=\"$filterString\", OU='$OuDn')"
...
function Find-AdGroups {
    ...
    try {
        Write-Log -Level INFO -Message "Recherche de groupes (filtre=\"$filterString\")"
...
$btnLoad.Add_Click({
    Write-Log -Level INFO -Message "Clic sur 'Charger les comptes' (OU='$($txtOu.Text.Trim())', motif='$($txtPattern.Text.Trim())')"
    ...
    # Forcer le rendu immédiat du log/progressBar avant l'appel bloquant vers l'AD,
    # sinon la fenêtre peut sembler figée le temps de la requête (surtout à distance).
    $form.Refresh()
    ...
    try {
        ...
        if ($accounts.Count -eq 0) {
            [System.Windows.Forms.MessageBox]::Show(
                "Aucun compte trouvé avec ces critères.`n`nVérifie :`n- le motif de nom (ex: svc_* doit correspondre au début du nom réel des comptes)`n- que le champ OU contient UN SEUL DN valide (une hiérarchie unique OU=...,OU=...,DC=...,DC=...), pas plusieurs OU juxtaposées`n- que tu as les droits de lecture sur cette OU",
                "Aucun résultat",
                [System.Windows.Forms.MessageBoxButtons]::OK,
        ...
        Write-Log -Level ERROR -Message "Échec du chargement : $($_.Exception.Message)"
        [System.Windows.Forms.MessageBox]::Show(
            "Erreur lors du chargement des comptes :`n`n$($_.Exception.Message)",
            "Erreur",
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Error
        ) | Out-Null
